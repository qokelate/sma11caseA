...
- (void)drawRect:(CGRect)rect
{
    ...
    if (_drawBlock) _drawBlock(context, rect);
}
...
// 画圆环
[dv setDrawBlock:^(CGContextRef context, CGRect rect) {
    const CGFloat w = 2.0f;
    const CGFloat r = MIN(rect.size.height, rect.size.width) / 2;
    const CGFloat x = rect.size.width/2;
    const CGFloat y = rect.size.height/2;
    const UIColor *color = WhiteColor;
    
    CGFloat rr,gg,bb,aa;
    [color getRed:&rr green:&gg blue:&bb alpha:&aa];
    CGContextSetRGBStrokeColor(context,rr,gg,bb,aa);
    CGContextSetLineWidth(context, w);
    CGContextAddArc(context, x, y, r-w/2, 0, 2*M_PI, 0);
